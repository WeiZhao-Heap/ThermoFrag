...
def _load_ok(pq: Path, score_col: str) -> Optional[pd.Series]:
    if not pq.exists():
        return None
...
def compute_c3(tf_root: Path, base_root: Path, target: str, k: int) -> dict:
...
def compute_c4(tf_root: Path, base_root: Path, target: str) -> dict:
...
def plot_bars(c3: pd.DataFrame, c4: pd.DataFrame, out_png: Path) -> None:
    ...
    for i, b in enumerate(baselines):
        ...
        axes[0].bar(x - 0.8 / 2 + width / 2 + (i + 1) * width, vals, width, label=b)
    ...
    for i, b in enumerate(baselines):
        ...
        axes[1].bar(x - 0.8 / 2 + width / 2 + (i + 1) * width, vals, width, label=b)
